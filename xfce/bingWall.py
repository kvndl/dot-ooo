...
paper_dir="/home/padfoot/.wallpapers/"
...
def download_image(img_link):
    ...
    if img_link is not None:  # ensure that we got a valid full-size jpeg background image.
        ...
        img_file = open(os.path.join(paper_dir, nowStr), 'wb')  # create file named with last part of the url.
...
def set_wallpaper():
    ...
    path = os.path.join(paper_dir, name)  # get image full path.

    # create command string to get list of monitors.
    cmd_get_mon = 'xfconf-query -c xfce4-desktop -l | grep workspace0/last-image'

    # execute command
    # mons = os.system(cmd_get_mon)
    
    # print(mons)

    # create command string to change last-image attribute which corresponds to desktop current background.
    cmd_set_last_image_0 = 'xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitorDP-0/workspace0/last-image -s {0}'.format(path)
    cmd_set_last_image_1 = 'xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitorDP-3/workspace0/last-image -s {0}'.format(path)
    cmd_set_last_image_2 = 'xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitorHDMI-0/workspace0/last-image -s {0}'.format(path)
    cmd_set_last_image_3 = 'xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitorDP-4/workspace0/last-image -s {0}'.format(path)

    # execute generated command above using os module although this can be done using subprocess module.
    os.system(cmd_set_last_image_0)
    os.system(cmd_set_last_image_1)
    os.system(cmd_set_last_image_2)
    os.system(cmd_set_last_image_3)
